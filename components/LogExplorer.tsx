// components/LogExplorer.tsx
import * as React from 'react';
import { useSearchParams, useLocation, useNavigate } from 'react-router-dom';
// FIX: Import GeneratedFilters to resolve type errors.
...
const LogExplorer: React.FC = () => {
    ...
    const applySavedSearch = (search: SavedSearch) => {
        ...
        // FIX: Defensively check types from saved search query to prevent state pollution.
        // Data from APIs is 'any' at runtime, so we must validate it.
    ...
    return (
      <div className="space-y-6">
        <h2 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Log Explorer</h2>

        {/* Filter Bar */}
        <Card>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="md:col-span-3">
                    <label className="text-sm font-semibold text-gray-600 dark:text-gray-300 block mb-2">Search & Filters</label>
        ...
        {histogramData.length > 0 && (
            <Card>
              <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Log Volume</h3>
              <div className="h-40">
                ...
                <table className="w-full text-sm mobile-card-view">
                    <thead className="border-b border-gray-200 dark:border-gray-700">
                        <tr>
                            ...
                            <th className="text-left p-2 w-24">Level</th>
                    ...
                    <tbody className="font-mono text-xs">
                        {loading ? (
                         ...
                         { !loading && logs.length === 0 && <tr><td colSpan={5} className="text-center p-8 text-gray-500">No logs found for the selected criteria.</td></tr>}
                    </tbody>
                </table>
            </div>
             <div className="flex justify-between items-center mt-4 text-sm p-4">
                <p>Showing page {pagination.page} of {pagination.totalPages} ({pagination.total.toLocaleString()} total logs)</p>
                <div className="flex gap-2">
